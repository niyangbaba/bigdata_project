...
object AnalysisLog {
  ...
  /**
    *
    * @param eventLogMap
    * @param requestParams /log.gif?en=e_pv&ver=1&pl=pc&os_n=win10&b_n=chrome
    */
  def handleRequsetParams(eventLogMap: mutable.Map[String, String], requestParams: String) = {
    //将log 根据 ? 切分
  ...
  def analysisLog(logText:String,ipRules:Array[IPRule]) = {
    ...
    if(StringUtils.isNotBlank(logText)){
      ...
      if(params.length == 4){
        ...
        //解析请求参数，并将参数加入到map中
        handleRequsetParams(eventLogMap,(params(3).trim))
      }
    }
    eventLogMap
  }
